...
func (h *KafkaHandler) MountHandlers(group *types.WorkerHandlerGroup) {
	group.Add("monitoring", h.handleMonitoring) // handling topic "monitoring"
}

// ProcessMessage from kafka consumer
func (h *KafkaHandler) handleMonitoring(eventContext *candishared.EventContext) error {
	trace, _ := tracer.StartTraceWithContext(eventContext.Context(), "MonitoringDeliveryKafka:HandleMonitoring")
	defer trace.Finish()
