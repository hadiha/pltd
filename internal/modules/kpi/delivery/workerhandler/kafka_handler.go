...
func (h *KafkaHandler) MountHandlers(group *types.WorkerHandlerGroup) {
	group.Add("kpi", h.handleKpi) // handling topic "kpi"
}

// ProcessMessage from kafka consumer
func (h *KafkaHandler) handleKpi(eventContext *candishared.EventContext) error {
	trace, _ := tracer.StartTraceWithContext(eventContext.Context(), "KpiDeliveryKafka:HandleKpi")
	defer trace.Finish()
