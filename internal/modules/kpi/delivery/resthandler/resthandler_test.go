...
import (
	...
	mockusecase "PLTD3/pkg/mocks/modules/kpi/usecase"
...
func TestRestHandler_getAllKpi(t *testing.T) {
	tests := []testCase{
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			kpiUsecase := &mockusecase.KpiUsecase{}
			kpiUsecase.On("GetAllKpi", mock.Anything, mock.Anything).Return(
				[]domain.ResponseKpi{}, candishared.Meta{}, tt.wantUsecaseError)
			mockValidator := &mockinterfaces.Validator{}
			mockValidator.On("ValidateDocument", mock.Anything, mock.Anything).Return(tt.wantValidateError)

			uc := &mocksharedusecase.Usecase{}
			uc.On("Kpi").Return(kpiUsecase)
			...
			err := handler.getAllKpi(echoContext)
...
func TestRestHandler_getDetailKpiByID(t *testing.T) {
	tests := []testCase{
		{
			name: "Testcase #1: Positive", wantUsecaseError: nil, wantRespCode: http.StatusOK,
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			kpiUsecase := &mockusecase.KpiUsecase{}
			kpiUsecase.On("GetDetailKpi", mock.Anything, mock.Anything).Return(domain.ResponseKpi{}, tt.wantUsecaseError)
			mockValidator := &mockinterfaces.Validator{}
			mockValidator.On("ValidateDocument", mock.Anything, mock.Anything).Return(tt.wantValidateError)
			...
			res := httptest.NewRecorder()
			echoContext := echo.New().NewContext(req, res)
			err := handler.getDetailKpiByID(echoContext)
			assert.NoError(t, err)
			assert.Equal(t, tt.wantRespCode, res.Code)
		})
	}
...
func TestRestHandler_createKpi(t *testing.T) {
	tests := []testCase{
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			kpiUsecase := &mockusecase.KpiUsecase{}
			kpiUsecase.On("CreateKpi", mock.Anything, mock.Anything).Return(tt.wantUsecaseError)
			mockValidator := &mockinterfaces.Validator{}
			mockValidator.On("ValidateDocument", mock.Anything, mock.Anything).Return(tt.wantValidateError)
			...
			err := handler.createKpi(echoContext)
			assert.NoError(t, err)
			assert.Equal(t, tt.wantRespCode, res.Code)
		})
	}
...
func TestRestHandler_updateKpi(t *testing.T) {
	tests := []testCase{
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			kpiUsecase := &mockusecase.KpiUsecase{}
			kpiUsecase.On("UpdateKpi", mock.Anything, mock.Anything, mock.Anything).Return(tt.wantUsecaseError)
			mockValidator := &mockinterfaces.Validator{}
			mockValidator.On("ValidateDocument", mock.Anything, mock.Anything).Return(tt.wantValidateError)
			...
			res := httptest.NewRecorder()
			echoContext := echo.New().NewContext(req, res)
			err := handler.updateKpi(echoContext)
			assert.NoError(t, err)
			assert.Equal(t, tt.wantRespCode, res.Code)
		})
	}
...
func TestRestHandler_deleteKpi(t *testing.T) {
	tests := []testCase{
		{
			name: "Testcase #1: Positive", wantUsecaseError: nil, wantRespCode: http.StatusOK,
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			kpiUsecase := &mockusecase.KpiUsecase{}
			kpiUsecase.On("DeleteKpi", mock.Anything, mock.Anything).Return(tt.wantUsecaseError)
			mockValidator := &mockinterfaces.Validator{}
			mockValidator.On("ValidateDocument", mock.Anything, mock.Anything).Return(tt.wantValidateError)
			...
			res := httptest.NewRecorder()
			echoContext := echo.New().NewContext(req, res)
			err := handler.deleteKpi(echoContext)
			assert.NoError(t, err)
			assert.Equal(t, tt.wantRespCode, res.Code)
		})
	}
