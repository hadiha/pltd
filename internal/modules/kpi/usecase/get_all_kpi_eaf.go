...
func (uc *kpiUsecaseImpl) GetKpiEaf(ctx context.Context, filter *domain.FilterKpi) (results []domain.ResponseKpi, meta candishared.Meta, err error) {
	trace, ctx := tracer.StartTraceWithContext(ctx, "KpiUsecase:GetKpiEaf")
	defer trace.Finish()

	var data []shareddomain.Kpi
	data, err = uc.repoSQL.KpiRepo().FetchAllEaf(ctx, filter)
	if err != nil {
