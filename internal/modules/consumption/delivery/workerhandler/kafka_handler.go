...
func (h *KafkaHandler) MountHandlers(group *types.WorkerHandlerGroup) {
	group.Add("consumption", h.handleConsumption) // handling topic "consumption"
}

// ProcessMessage from kafka consumer
func (h *KafkaHandler) handleConsumption(eventContext *candishared.EventContext) error {
	trace, _ := tracer.StartTraceWithContext(eventContext.Context(), "ConsumptionDeliveryKafka:HandleConsumption")
	defer trace.Finish()
